...
export function ChatInterface() {
  ...
  const handleBudgetSelect = async (budget: string) => {
    // Ensure we have all the user data when creating the summary
    const completeUserData = { ...userData, budget };
    ...
    try {
      const response = await fetch("/api/recommendations", {
        ...
        body: JSON.stringify({
          ...
          budget: budget,
          focusCountry: completeUserData.country, // Ensure country-specific results
        }),
      });
